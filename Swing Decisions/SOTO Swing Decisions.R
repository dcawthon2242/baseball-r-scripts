...
pbp_2022_2024_pitch_call <- filter(soto_pbp_2022_2024, description %in% pitch_call) %>%
...
soto_pbp_2022_2024 <- soto_pbp_2022_2024 %>%
...
filtered_pbp_2022_2024_swing <- filter(soto_pbp_2022_2024, description %in% swing_types)
...
soto_pbp_2022_2024$xWhiff <- pred_probs
...
# Filter the data for the specified pitch calls to create training dataset
filtered_pbp_2022_2024_contact <- filter(soto_pbp_2022_2024, description %in% contact_types)
...
one_hot_pbp_2022_2024_contact <- pbp_2022_2024_contact %>%
  select(plate_x, plate_z, balls, strikes, in_play)

# Prepare features for full dataset (excluding in_play column)
one_hot_pbp_2022_2024_full <- soto_pbp_2022_2024 %>%
  select(plate_x, plate_z, balls, strikes)
...
# Add predictions to the original dataset
soto_pbp_2022_2024$xIn_Play <- pred_probs
...
pbp_2022_2024_in_play <- soto_pbp_2022_2024 %>%
...
relevant_features <- c("plate_x", "plate_z", "strikes", "balls")
...
soto_pbp_2022_2024$swing <- ifelse(
...
# Prepare features for training
one_hot_pbp_2022_2024 <- soto_pbp_2022_2024 %>%
  select(plate_x, plate_z, balls, strikes, swing)
...
# Add predictions to the original dataset
soto_pbp_2022_2024$xSwing <- pred_probs
...
soto_pbp_2022_2024$xContact <- 1 - soto_pbp_2022_2024$xWhiff
soto_pbp_2022_2024$xFoul <- 1 - soto_pbp_2022_2024$xIn_Play

# Ensure pbp_2022_2024 is a data frame
soto_pbp_2022_2024 <- as.data.frame(soto_pbp_2022_2024)
...
soto_pbp_2022_2024$soto_rv <- soto_pbp_2022_2024$run_value_change_actual - soto_pbp_2022_2024$run_value_change
